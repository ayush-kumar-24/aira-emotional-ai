...
async def analyze_voice_endpoint(
    ...
    - Transcription for chat history
    - Audio response for playback
    """
    ...
    try:
        ...
        logger.info(f"😊 Voice Emotion: {voice_emotion}")
        
    except Exception as e:
        logger.exception("Error in voice analysis")
    ...
    if crisis_data["is_crisis"]:
        logger.warning(f"🚨 Crisis detected: {transcription}")
        
        # Generate crisis audio
    ...
    # ========================================
    # STEP 3: Text Emotion + LLM Response
    # ========================================
    try:
    ...
    try:
        ...
        logger.info(f"✅ Audio generated: {response_audio_url}")
    ...
    # ========================================
    return JSONResponse(content={
        "transcription": transcription,           # For chat history
        "voice_emotion": voice_emotion,
        "text_emotion": text_emotion,
        "confidence": voice_result["confidence"],
        "response_text": llm_response,            # For chat history
        "response_audio_url": response_audio_url, # For playback
        "features": voice_result["features"],
        "all_scores": voice_result["all_scores"]
    })